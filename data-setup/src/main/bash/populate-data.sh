...
log "Waiting for keycloak to start to populate test data. Wait for $KEYCLOAK_READINESS_CHECK_URL to return Status 303 ..."

while true; do
  http_status=$(curl -s -o /dev/null -w '%{http_code}' $KEYCLOAK_READINESS_CHECK_URL)
  echo "HTTP Status: $http_status"
  if [[ "$http_status" == "303" ]]; then break; fi
  sleep 10
done
